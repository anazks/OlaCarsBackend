...
const bulkAddVehicles = async (req, res) => {
    try {
            ...
            try {
                ...
                if (existingVehicle) {
                    ...
                    if (isNARegistration(currentReg)) {
                        ...
                        const updatedVehicle = await updateVehicleService(existingVehicle._id, updateData);
        ...
        return res.status(statusCode).json({
            ...
            message: `${results.created.length} vehicle(s) created/updated, ${results.errors.length} error(s).`,
            data: results,
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message });
