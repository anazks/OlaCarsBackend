...
exports.login = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, ...result, data: result.user });
  } catch (error) {
...
exports.addWorkshopManager = async (req, res) => {
  try {
    ...
    const newManager =
      await WorkshopManagerService.createWorkshopManagerService(data);
...
exports.getWorkshopManager = async (req, res) => {
  try {
    ...
    if (req.user.role === "COUNTRYMANAGER" && req.user.country) {
      const countryBranches = await BranchService.getAll({
    ...
    const result = await WorkshopManagerService.getAllWorkshopManagersService(
      queryParams,
      options,
    );
...
exports.getWorkshopManagerById = async (req, res) => {
  try {
    const result = await WorkshopManagerService.getWorkshopManagerByIdService(
      req.params.id,
    );
...
exports.editWorkshopManager = async (req, res) => {
  try {
    const payload = {
      ...
      modifierRole: req.user.role,
    };
    const result = await WorkshopManagerService.updateWorkshopManagerService(
      req.params.id,
      payload,
    );
    return res.status(200).json({ success: true, data: result });
...
exports.deleteWorkshopManager = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "Workshop Manager deleted successfully",
      });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    return res
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const result = await WorkshopManagerService.changePasswordService(
      req.params.id,
      currentPassword,
      newPassword,
    );
...
exports.refreshManagerToken = async (req, res) => {
  try {
    ...
    const result =
      await WorkshopManagerService.refreshSessionService(refreshToken);
    return res.status(200).json({ success: true, ...result });
  } catch (error) {
    const statusCode = error.statusCode || 401;
    return res
