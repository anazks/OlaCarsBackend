...
exports.login = async (email, password) => {
    ...
    const accessToken = jwt.sign(
        { id: staff._id, role: 'FINANCESTAFF', branchId: staff.branchId },
...
exports.create = async (data) => {
    ...
    if (finalPermissions.length === 0) {
       ...
       const template = await RoleTemplate.findOne({ roleName: 'FINANCESTAFF' });
...
exports.update = async (id, body) => {
    ...
    if (!updated) throw new AppError('Finance Staff not found', 404);
    return updated;
};
...
exports.remove = async (id) => {
    const result = await FinanceStaff.findByIdAndUpdate(id, { isDeleted: true }, { new: true });
