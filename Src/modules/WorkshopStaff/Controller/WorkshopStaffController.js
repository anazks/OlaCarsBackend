...
const login = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      ...result,
      data: result.user, // Ensure frontend can access the logged-in user payload
    });
  } catch (error) {
...
const getWorkshopStaffById = async (req, res) => {
  try {
    ...
    if (!staff)
      return res
        .status(404)
        .json({ success: false, message: "Workshop Staff not found" });
...
const editWorkshopStaff = async (req, res) => {
  try {
    ...
    const updatedStaff = await WorkshopStaffService.update(
      req.params.id,
      payload,
    );
...
const changePassword = async (req, res) => {
  try {
    ...
    const result = await WorkshopStaffService.changePassword(
      req.params.id,
      currentPassword,
      newPassword,
    );
...
const deleteWorkshopStaff = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Workshop Staff deleted successfully" });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    return res
      .status(statusCode)
