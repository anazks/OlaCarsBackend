...
async function inspect() {
    try {
        ...
        const driver = await Driver.findOne({ "personalInfo.fullName": /LIGIA ESPINO/i });
        if (!driver) {
        ...
        const invoices = await Invoice.find({ driver: driver._id, invoiceType: 'RENTAL' }).sort({ dueDate: 1 });
